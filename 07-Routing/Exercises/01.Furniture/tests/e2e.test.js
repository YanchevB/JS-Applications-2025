...
describe('E2E tests', function () {
  ...
  describe('CRUD', () => {
    ...
    it('Delete makes correct API call for logged in user', async () => {
      ...
      await loginUser();
      const item = mockData.catalog[0];
      const { get, del } = await handle(endpoints.details(item._id));
      get(item);
      ...
      await page.click(`.card:has-text("${item.description}") >> text=Details`, {
        timeout: interval,
      });

      page.on('dialog', (dialog) => dialog.accept());

      await page.waitForSelector('a:has-text("Delete")', { timeout: 3000 });
      await Promise.all([
        onResponse(),
